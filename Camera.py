...
Description: Streaming Camera with zoom function
"""
...
class Camera:
    ...
    def __setup(self):
        # Prepare the camera settings, button manager.
    ...
    def event(self, i):
        # Function to Button Event
        # Just write a button event in that function.
        # i is indexed according to the order of the buttons. (0 to n)
        pass
        return i
    ...
    def show(self):
        ...
        while True:
            ...
            if frame is not None:
                ...
                cv2.imshow('UI-OPEN-CV', frame)
                cv2.setMouseCallback('UI-OPEN-CV', self.mouse_callback)
            key = cv2.waitKey(1)
            if key == ord('q'):
                # q : close
                self.release()
